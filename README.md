...
The final trained Random Forest pipeline was serialized using `joblib`.
Due to GitHub file size limitations, the serialized model file
(`final_random_forest_pipeline.joblib`) is not included in this repository.

The model can be regenerated by running the notebook end-to-end,
which ensures full reproducibility of the results.
